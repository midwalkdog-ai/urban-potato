...
export default function OwnerJobs({ user }: { user: User }) {
  const [jobs] = useState(() => generateMockJobs(12));
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '4px', padding: '18px' }}>
        {jobs.map(job => (
          <div key={job.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ background: job.status === 'completed' ? 'var(--green)' : 'var(--amber)', color: job.status === 'completed' ? '#fff' : '#000', padding: '2px 8px', borderRadius: '3px', fontSize: '10px', fontWeight: 600 }}>{job.status}</span>
          </div>
        ))}
      </div>
    </div>
